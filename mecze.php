	...
	<head>
		...
		<script>
			function załadujStatsy() {
				...
				$(this).find('.meczetabela').append($(tr1).load("statystyki.php", {id: $(this).data("id")}, function() {
					var statsyAE = document.getElementById("statsyAspan");
					var a = statsyAE.innerHTML;
					a = a.replace(/&lt;br&gt;/gi,'<br>');
					statsyAE.innerHTML = a;
					var statsyBE = document.getElementById("statsyBspan");
					a = statsyBE.innerHTML;
					a = a.replace(/&lt;br&gt;/gi,'<br>');
					statsyBE.innerHTML = a;
				}));
				$(".wynikA").show();
				$(".wynikB").show();
				$(this).one("click",usun);
		...
		<script>
			$(document).ready(function(){
				...
				var slides = document.getElementsByClassName("nazwagraczaA");
				for(var i = 0; i < slides.length; i++)
				{
				   slides.item(i).innerHTML =  slides.item(i).innerHTML.replace(/&lt;br&gt;/gi,'<br>');
				}
				
				var slides = document.getElementsByClassName("nazwagraczaB");
				for(var i = 0; i < slides.length; i++)
				{
				   slides.item(i).innerHTML =  slides.item(i).innerHTML.replace(/&lt;br&gt;/gi,'<br>');
				}
				
				var slides = document.getElementsByClassName("nazwagraczakartkiA");
				for(var i = 0; i < slides.length; i++)
				{
				   slides.item(i).innerHTML =  slides.item(i).innerHTML.replace(/&lt;br&gt;/gi,'<br>');
				}
				
				var slides = document.getElementsByClassName("nazwagraczakartkiB");
				for(var i = 0; i < slides.length; i++)
				{
				   slides.item(i).innerHTML =  slides.item(i).innerHTML.replace(/&lt;br&gt;/gi,'<br>');
				}
					
				/*document.querySelectorAll('nazwagraczaA').forEach(function(){
					var a = $(this).innerHTML;
					alert(a);
					$(this).innerHTML = $(this).innerHTML.replace(/&lt;br&gt;/gi,'<br>');
				});
				document.querySelectorAll('nazwagraczaB').forEach(function(){
					$(this).innerHTML = ($(this).innerHTML).replace(/&lt;br&gt;/gi,'<br>');
				});*/
			});
		</script>
...
$sql = "SELECT d.id, d.nazwa, d.logo, m.teamaid, m.teambid FROM mecze m, druzyny d WHERE m.teamaid = d.id OR m.teambid = d.id";
$result = $conn->query($sql);
$druzyny = array(
    array (),
    array (),
...
if ($result->num_rows > 0) {
	$i = 0;
	while ($row = $result->fetch_assoc()) {
		$druzyny[$row["id"]]["nazwa"] = $row["nazwa"];
		$druzyny[$row["id"]]["logo"] = $row["logo"];
	}
}
					...
					"<tr style=''>" .
						"<td class='teamalogo' style=''>" .
							"<a style='display:block; width:100%; height:200px;'>" .
								"<img class='logoa' src='" . $druzyny[$row["teamaid"]]["logo"] . "')
							</a>" .
						"</td>" .
						...
						"<td class='teamblogo' style=''>" .
							"<a style='display:block; width:100%; height:200px;'>
								<img class='logoa' src='" . $druzyny[$row["teambid"]]["logo"] . "');
							</a>" .
						"</td>" .
					"</tr>" .
					"<tr>" .
						"<td class='nazwateamu'>" . $druzyny[$row["teamaid"]]["nazwa"] . "</td>" .
						"<td></td>" .
						"<td></td>" .
						"<td></td>" .
						"<td class='nazwateamu'>" . $druzyny[$row["teambid"]]["nazwa"] . "</td>" .
					"</tr>";
